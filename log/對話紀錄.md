...
您需要確保 Flask 應用程式正確啟動。請檢查 `main.py` 中的啟動代碼是否如下所示：
...
Flask 的內建開發服務器不適合生產環境。根據您的 `requirements.txt` 中已包含 `gunicorn`，建議修改 Dockerfile 中的啟動命令：
...
確保 `main.py` 中正確導入了 `excel_json_processor.py`：

```python
from utils.excel_json_processor import format_shipping_document
```

並且確保 `utils` 目錄中有 `__init__.py` 檔案，使其成為可導入的套件。
...
/app
  ...
  └── utils/
      ├── __init__.py
      └── excel_json_processor.py
```

### 6. 重新部署

修改完成後，重新提交代碼並部署。如果您使用 Google Cloud Build 和 Cloud Run，可以執行：

```bash
gcloud builds submit --tag gcr.io/YOUR_PROJECT_ID/excel-flask
gcloud run deploy excel-flask --image gcr.io/YOUR_PROJECT_ID/excel-flask --platform managed
```

如果問題仍然存在，請查看 Cloud Run 的詳細日誌，了解具體的錯誤信息。您可以通過錯誤訊息中提供的日誌 URL 訪問這些日誌。
